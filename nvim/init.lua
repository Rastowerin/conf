...
vim.fn.sign_define(
	...
	{ text = "◆", texthl = "DapBreakpointCondition", linehl = "DapBreakpointLine", numhl = "DapBreakpointCondition" }
)
vim.fn.sign_define(
...
local function set_dap_colors()
	...
	vim.api.nvim_set_hl(0, "DapBreakpointLine", { bg = bg2 })
	vim.api.nvim_set_hl(0, "DapBreakpointCondition", { fg = yellow })
	vim.api.nvim_set_hl(0, "DapBreakpointRejected", { fg = red })
	vim.api.nvim_set_hl(0, "DapLogPoint", { fg = blue })
	vim.api.nvim_set_hl(0, "DapStopped", { fg = cyan })
	vim.api.nvim_set_hl(0, "DapStoppedLine", { bg = "#2a3158" })
end
set_dap_colors()
vim.api.nvim_create_autocmd("ColorScheme", { callback = set_dap_colors })
		...
		config = function()
			...
			vim.keymap.set("n", "<F5>", function()
				...
				if file:match("_test%.go$") then
					...
					require("dap").run({ type = "go", name = "Debug", request = "launch", program = file })
				end
			...
			vim.keymap.set("n", "<leader>du", function()
				if dap.session() then
					...
					vim.notify("Нет активной DAP-сессии", vim.log.levels.WARN)
				end
